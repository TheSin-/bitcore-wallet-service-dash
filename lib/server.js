...
WalletService.prototype.savePreferences = function(opts, cb) {
  ...
  var preferences = [{
    ...
    name: 'unit',
    isValid: function(value) {
      return _.isString(value) && _.contains(['trc', 'bit'], value.toLowerCase());
    },
  }];
    ...
    self._getUtxosForCurrentWallet(null, function(err, utxos) {
      ...
      var info = {
       ...
       if(opts.isInstantSend===true){
        ...
        if(!validUTXO)
            return cb(new Error('InstantSend requires all Terracoin you are sending (transaction inputs) to be held for about 15-20 minutes (minimum 6 network confirmations). Please wait before using InstantSend.'));
       }
      ...
      self._getFeePerKb(wallet, opts, function(err, feePerKb) {
        ...
        if(opts.isInstantSend===true){
             ...
             var feePerInput = 20000;//Fee for IS are 0.0002 * INPUTS 
 ...
 * @param {Object} opts
 * @param {string} [opts.network = 'livenet'] - The Terracoin network for this transaction.
